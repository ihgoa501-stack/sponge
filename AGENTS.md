...
- **Mission**: Build the most cost-effective AI agent harness — **use any model, pay less every time**. Competing with Codex, GitHub Copilot, Cursor, OpenHands, and similar tools.
- **Status**: Design & documentation complete. Ready for Phase 0 implementation. No runtime code written yet.
- **Design lineage**: Architecture informed by deep research of Codex (+ Agent SDK), Cursor, GitHub Copilot (HyDRA), OpenHands, LangChain/LangGraph/Deep Agents, CrewAI, Reasonix, and mid-2026 LLM pricing/caching landscape.
...
- **Key differentiator**: First harness where **cost optimization is the infrastructure, not the routing decision**. The model never changes; the cost per call does. P95 budgeting, circuit breakers with pre-call cost estimation, 5-layer context compression, prompt caching with cross-provider adaptive strategy, result/semantic caching, local preprocessing → cloud execution. **Self-tuning: the harness watches its own cost data and optimizes itself — the more you use it, the more it saves.** Native MCP support, streaming by default, three-tier approval gates, project-level long-term memory, and multimodal input (images/PDFs).
...
| **Harness** | Predefined tools, prompts, subagents, planning, filesystem, context management | Deep Agents SDK, Codex Agent SDK, Manus |

A **harness** is an opinionated, batteries-included layer above the runtime — it provides the agent loop, tool execution, context management, sub-agent spawning, memory, sandboxing, and planning infrastructure. The harness should be **as thin as possible** — Codex's core loop is ~30 lines. 95% of complexity lives in infrastructure (tools, context, permissions, sandboxing), not in the agent loop itself.
...
│   ├── anthropic_provider.py      # Anthropic Codex provider
...
| **Static configs** | Codex, Cursor, Copilot all ship with hardcoded thresholds |
...
{"tokens_in":3420,"tokens_out":812,"cache_hit":true,"cache_write":true,
 "cost":0.018,"latency_ms":420,"model":"Codex-opus-4-7"}
...
|-------|-------|--------|------------|-------|
| **Codex Opus 4.7** | $5.00 | $25.00 | $0.50 | New tokenizer: +35% tokens |
| **GPT-5.5** | $5.00 | $30.00 | $0.50 | Default 24h cache retention |
| **Codex Sonnet 4.6** | $3.00 | $15.00 | $0.30 | Context awareness built-in |
| **GPT-5.4** | $2.50 | $15.00 | $0.25 | Strong code-gen |
| **Codex Haiku 4.5** | $1.00 | $5.00 | $0.10 | Context awareness built-in |
...
**Sponge's default model**: Codex Opus 4.7 ($5/$25 per MTok) or GPT-5.5 ($5/$30 per MTok).
...
- **Context Summarization**: raw exploration data condensed locally before best model sees it
...
($0.014 total input), then return a 1K-token summary. The best model sees 1K
tokens instead of 100K — a 100x reduction in expensive tokens. The saved 99K
tokens on Opus 4.7 would have cost $0.50.

### Coordination Patterns

| Pattern | Description | Where Best Model Sits |
|---------|-------------|----------------------|
| **Dispatch-Condense** | Sub-agents explore in parallel → best model synthesizes condensed results | Best model as synthesizer |
| **Draft-Review** | Local/cheap model drafts → best model polishes | Best model as editor |
| **Generator-Validator** | Sub-agent generates → best model quality-checks | Best model as gatekeeper |
| **Lead-Worker** | Best model decomposes → workers execute → best model integrates | Best model as orchestrator |

### Design Rules (from Codex's proven patterns)
...
5. **Condensed outputs**: sub-agent returns structured summary, not raw logs.
   The harness forces condensation before results reach the best model.
...
**Why MCP?** MCP (Model Context Protocol) is the industry standard — Codex, Cursor, Copilot all support it. Building a custom protocol forces every tool author to write two integrations.
...
**Project memory (`.sponge/memory.toml`)** acts like Codex's `AGENTS.md` or Cursor's `.cursorrules`:
...
|------|-----------|------|
| **Codex** | Proven architecture, context awareness, sub-agents, Agent SDK | No built-in cost optimization; users manually choose cheaper models |
...
| **Reasonix** | Cache-first design, 99.8% cache hit rate on DeepSeek, 93.9% cheaper than Codex | DeepSeek-only; quality ceiling lower than Opus |
| **Codex Agent SDK** | Same engine as Codex, library form | No built-in cost optimization; Anthropic-only |
...
agent harness that never downgrades the model but slashes cost through infrastructure
...
### Cost-First Mindset
- The best model is the default. Never reduce quality to save money.
...
- LLM SDK: anthropic (for Codex), openai, with provider-agnostic abstraction layer
- CLI framework: typer
...
- Benchmark tests comparing cost-per-task against Codex, Copilot, and Cursor.
